...
One such website currently facing server scaling problems is {\textsf iFixit}, a moderately sized website that is ``the free repair manual that you can edit''\cite{ifixitDotCom}.
...
% TODO: Rewrite this (or not): Additionally, anyone can edit the content due to it being a wiki.
...
In addition to their main website, {\textsf iFixit} runs a Software as a Service (SaaS) that provides technical documentation to anyone who needs step-by-step instructions.
...
They get all of their data from one master database machine running the Percona build of MySQL which handles all reads and writes.
...
After the specified time has passed, the entry is expired and future requests for it will result in a cacheresult in a cache  miss.
...
{\textsf Memcached} is by far the most popular web caching system; it is currently in use by Facebook, Wikipedia, YouTube, and countless others \cite{memcachedDotOrg}.
