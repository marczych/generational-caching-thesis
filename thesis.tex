...
% TODO: Fix weird spacing around code figures.
...
\textsf{Memcached} is an example of a dynamic cache daemon which is well-suite for dynamic websites.
...
This is typically because the request URL only specifies the primary key because the {\tt modified\_date} isn't known externally.
...
For a {\tt guideid} and {\tt langid} of {\tt 1234} and {\tt en}, respectively, the cache is checked for the {\tt Guide} object using {\tt Guide/1234-en} as the cache key.
...
The full cache key can then be constructed, {\tt Guide/1234-en-1368164354.918344} for example, and used to fetch the rest of the content.
...
Rather than including the step number in the cache key, a placeholder value can be used in place of the step number.
The placeholder is replaced with the actual step number before it is output to the user.
...
% TODO: Example call stack or something.
...
{\tt batchCache} calls \textsf{Memcached} {\tt getMulti} with the provided array of cache keys.
For each cache miss, output buffering is started and the supplied rendering function is called with the data associated with the cache key.
Output buffering is then closed and the resulting HTML is stored in the cache and added to the array of resulting HTML.
Finally, every fragment of HTML is output to {\tt stdout} in the same order as they were provided.
% TODO: Figure for batchCache.
...
$steps = cacheGetAndSet($cacheKey, function() use ($guide) {
   return dbQuery("
      SELECT *
      FROM steps
      WHERE guideid = ? AND langid = ?",
...
% TODO: Include detailed example throughout section explaining how this all works for guides.
...
The most general value we will compare is page response time, the total amount of time the request spent on our servers.
Response time should decrease because more data is cached and can be returned faster than before.
Similarly, HTML generation time should decrease because fragments of HTML are now being cached.

The interesting values for \textsf{Memcached} stats are {\tt get} count, hit rate, and time spent in cache calls.
The {\tt get} count will likely increase because cache entries are finer-grained.
Cache hit rate will vary drastically because pages with uncached content will have a large number of misses while pages with cached content will have a large number of hits.
...
Finally, the interesting values for \textsf{MySQL} are query count and total query time.
Query count should go down because the cache should serve more requests that previously were handled by the database.
Additionally, more content is retrieved in batch so there are fewer queries which reduces network overhead.
