...
%\section{Hypothesis}
% TODO: Write. If anything take stuff from Measured Values but probably not necessary.
...
% TODO: Move most of this to background?
Unfortunately, \textsf{Memcached} {\tt get}s are very skewed between the two strategies because of \textit{wiki text} rendering.
Wiki text is user-submitted text that is rendered to HTML before being displayed to the user.
This transformation is cached because wiki text parsing and rendering is a slow and expensive process.
Currently, \textsf{iFixit} does a \textsf{Memcached} {\tt multiGet} call to get the rendered HTML for the wiki text of all bullet points of a guide.
This results in a large number of gets for the control that are nearly all hits.
