...
% TODO: Overview of HTML and query caching?
...
% TODO: Example use of this? Remove this?
...
\subsection{Caching Ad Hoc Queries}
The same approach can be used for caching HTML and ad hoc queries.
As long as the results of the query can only change when the ORM object's cache key changes, the cache entries will be invalidated as necessary.
...
\begin{minted}{php}
<?php
$cacheKey = $guide->cacheKey() . "_steps";
$steps = cacheGetAndSet($cacheKey, CACHE_FOREVER, function() use ($guide) {
   return dbGet("
      SELECT *
      FROM steps
      WHERE guideid = ? AND langid = ?",
