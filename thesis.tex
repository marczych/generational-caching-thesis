...
\chapter{Design}
\label{design}
In this chapter, we explain the high-level design of the generational caching strategy implemented for iFixit.

\section{What is being cached}
...
\section{ORM Caching}
The primary key uniquely identifies a tuple in the database.
This can be used with the ORM to select a single record.
...
The entire database tuple is cached using the primary key to avoid hitting the database as much as possible.
...
In this case there is no benefit to using generational caching when retrieving the object.
However, it can be used in cache keys for data that depends on it to avoid cache invalidation.
The amount of extra data loaded after the primary table is fetched to make the overhead of generational caching worthwhile varies on a case-by-case basis.

Additionally, all of this happens for multiple objects as well.
That is to say that multiple objects can be fetched in one call and the same number of database queries and memcache gets are performed.
