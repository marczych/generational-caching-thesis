...
% TODO: Update this value with fresh results.
Our implementation yielded a 20\% improvement in page response time by caching database queries and fragments of HTML.
\end{abstract}
...
Although application machines can be scaled easily, each one still needs to access the same underlying data which is typically a single database machine.
...
All application machines access a single \textsf{Memcached} instance with 4GB of space available.
...
The problem ease-of-use addresses is that manually controlling the cache is tedious and error-prone.
...
For these reasons, \textsf{Memcached} is well-suited for web application caching.
...
In Section~\ref{design} we explain the high-level design of the caching strategy implemented for \textsf{iFixit}.
In Section~\ref{implementation} we go over the details of implementing the design in \textsf{iFixit}'s code base.
...
In general, \textsf{Memcached} misses should increase because cache entries aren't set to expire and the only misses will be for new or changed content.
...
However, miss count is less biased once most content is cached which is why miss count is a more useful measurement.
...
Our generational caching implementation has a large number of misses because cache misses result in more cache {\tt get}s which are also misses.
...
\begin{description}
   \item[R1] The caching solution is largely transparent to the developer.
             The developer uses the ORM object to construct cache keys and any cache entries using that object are invalidated when the underlying data changes.
   \item[R2] The caching strategy allows for finer-grained caching without added complexity.
...
% TODO: Write if we want future work.
%\section{Future Work}
