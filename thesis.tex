...
One such company is iFixit, a provider of free, publicly-editable repair manuals.
Like many other websites, iFixit uses Memcached for caching to decrease database load and improve response time.
However, the caching strategy used is a very ad hoc one and therefore can be greatly improved.
In this thesis we investigate caching strategies and find that generational caching fits iFixit's needs best.
We then describe the process of implementing generational caching in iFixit's web site, report problems encountered, and provide before and after statistics.
\end{abstract}
...
{\textsf iFixit} currently employs an ad hoc caching strategy that relieves some server load and decreases response time, however, the effectiveness of the cache can be greatly improved.
...
{\textsf iFixit}'s most popular content is their step-by-step repair guides.
An {\textsf iFixit} guide consists of meta data about the guide such as title, summary, introduction, author, time required, and difficulty.
...
All of {\textsf iFixit}'s content is dynamic.
...
In particular, {\textsf iFixit} is known for its device ``teardowns.''
...
In addition to their main website, {\textsf iFixit} runs a Software as a Service (SaaS) platform for technical documentation platform to anyone who needs step-by-step instructions.
...
{\textsf iFixit} runs two load balancers - one for {\textsf iFixit} and one for their SaaS.
At any given time there are typically four to eight application machines that run {\textsf iFixit}'s core software which is written in PHP.
...
In section 3.1 we explain the high-level design of the generational caching strategy implemented for {\textsf iFixit}.
