...
This removes any variables from the amount and type of site traffic that occurs naturally.
Results should be very close to each other for the same experiment which demonstrates the validity of the testing environment.
First an unaltered log of site traffic will be replayed on both systems to emulate real world traffic patterns.
Additionally, this log will be extrapolated to simulate increased site traffic.
Multiple levels of increased site traffic will be run on both systems as a comparison of their merits of scalability.
This method will yield an approximate value for requests per second that each system can handle.
...
The contents of the database will be populated with this dump to ensure a similar experiment.
The only problem with this approach is that populating a database from a sql dump may take a significant amount of tme depending on the size of the data.
